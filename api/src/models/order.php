...
function m_Order_assign(int $orderId, int $worker_id)
{
    ...
    if (!$rows || $error) {
        return response_error('Order for assign not found' . $error);
...
function m_Order_generate_transaction_id(int $orderId)
{
    ...
    if (!$rows || $error) {
        return response_error('Opened order not found' . $error ?? null);
...
function m_Order_list(int $page)
{
    ...
    if (!$orderIds) {
        ...
        $startId = 0;
        if ($page) {
            $startId = (PAGE_SIZE * $page);
        }

        $query = 'SELECT id FROM orders WHERE id>? AND status=? LIMIT ?';
        $s = mysqli_prepare(db_getConnection('order'), $query);

        mysqli_stmt_bind_param($s, 'iii', $startId, $status, $pageSize);
        mysqli_stmt_execute($s);
        $error = mysqli_stmt_error($s);
        $result = mysqli_stmt_bind_result($s, $id);

        if (!$result || $error) {
            return response_error('Orders not found' . $error);
        }
